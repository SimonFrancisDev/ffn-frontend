...
const ActivationCenterPage = () => {
  ...
  const applySecuredReferralToState = useCallback((referralData) => {
    ...
    setResolvedReferrerStatus(
      referralData.restored
        ? `Existing referrer restored. Secured for ${getReferralDaysLeftText(referralData.daysLeft)}.`
        : `Referrer secured for ${getReferralDaysLeftText(referralData.daysLeft)}.`
    )
  }, [])
      ...
      try {
        ...
        if (existingSecureReferral?.locked) {
          ...
          if (
            ...
            setResolvedReferrerStatus(
              `A referrer is already secured for ${getReferralDaysLeftText(existingSecureReferral.daysLeft)}. New referral links cannot override it.`
            )
          }
  ...
  const handleRegisterFromModal = async () => {
    try {
      let finalSecureReferral = securedReferral

      if (!finalSecureReferral?.locked) {
        const manualRef = referrerInputDisplay?.trim()

        if (manualRef) {
          finalSecureReferral = await secureReferralWithSystem(manualRef, 'manual_input')
        } else {
          finalSecureReferral = await secureReferralWithSystem('FIN-FREEDOM', 'system')
        }
      } else {
        const restoredReferral = await fetchSecuredReferral()
        if (restoredReferral?.locked) {
          finalSecureReferral = restoredReferral
          applySecuredReferralToState(restoredReferral)
        }
      }
            ...
            <div className="activation-overlay" role="dialog" aria-modal="true">
              <div className="activation-modal">
                ...
                <div className="referrer-input-group">
                  ...
                  <input
                    type="text"
                    className={`referrer-input ${securedReferral?.locked ? 'is-locked' : ''}`}
                    placeholder="Paste referral ID, referral link, or wallet address"
                    value={referrerInputDisplay}
                    readOnly={Boolean(securedReferral?.locked)}
                    onChange={(e) => {
                      if (securedReferral?.locked) return
                      ...
                      if (ethers.isAddress(value)) {
                        ...
                        setResolvedReferrerStatus('Wallet address detected. It will be secured before registration.')
                      } else {
                        setRegistrationReferrer('')
                        setResolvedReferrerStatus('Referral ID will be resolved and secured before registration.')
                      }
                    }}
                  />
                  ...
                  <p className="referrer-hint referrer-hint--important">
                    {securedReferral?.locked
                      ? `This referrer is secured for ${getReferralDaysLeftText(securedReferral.daysLeft)} and cannot be replaced during this period.`
                      : 'Your referrer will be verified and secured before registration.'}
                  </p>
                </div>
...
//                   </p>
//                   <p className="referrer-hint referrer-hint--important">
//                     This placement cannot be changed later unless you register another wallet.
//                   </p>
//                 </div>
