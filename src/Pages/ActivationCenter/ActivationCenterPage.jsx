...
const getOrCreateActivationVisitorId = () => {
...
// ADDED: normalizeReferralInput helper
...
// REPLACED: extractReferralFromUrl
...
const ActivationOrbitGalaxy = ({ level, viewer, snapshot }) => {
  const orbitType = snapshot?.orbitType || levelToOrbitType[level]
  const structure = getOrbitStructure(orbitType)
  const positions = Array.isArray(snapshot?.positions) ? snapshot.positions : []
  const summary = snapshot?.orbitSummary || {}

  const totalPositions = orbitTypeConfig[orbitType]?.positions || 4
  const filledCount = positions.filter((item) => item?.occupant && item.occupant !== ethers.ZeroAddress).length
  ...
  const stageSize = orbitType === 'P39' ? 520 : orbitType === 'P12' ? 460 : 380
  const centerX = stageSize / 2
  const centerY = stageSize / 2
  const coreSize = orbitType === 'P39' ? 78 : 88

  const ringRadiiPx = {
    1: orbitType === 'P4' ? 120 : 105,
  ...
  return (
    <div className="activation-orbit-preview">
      <div className="activation-orbit-summary">
        <div>
          <span>Orbit</span>
        ...
        <div>
          <span>Filled</span>
          <strong>{filledCount}/{totalPositions}</strong>
        </div>
        <div>
          <span>Current cycle</span>
          <strong>{Number(summary.totalCycles || 0) + 1}</strong>
        </div>
        <div>
          <span>Total earned</span>
          <strong>{summary.totalEarned || '0'} USDT</strong>
        </div>
      </div>

      <div className="activation-galaxy-container">
        <div
          className="activation-galaxy-stage"
          style={{
            width: stageSize,
            height: stageSize,
          }}
        >
          <div
            className="activation-orbit-core"
            style={{
              width: coreSize,
              height: coreSize,
            }}
          ...
          {structure.lines.map((lineNum) => {
            ...
            const linePositions = structure.positions[lineNum] || []
            const arrivals =
              lineNum === 1 ? lineCounts.line1 : lineNum === 2 ? lineCounts.line2 : lineCounts.line3

            const lineFilled = positions.filter((pos) => Number(pos.line || 1) === lineNum && pos.occupant).length

            return (
              <div
                key={`ring-${lineNum}`}
                className={`activation-orbit-ring line${lineNum}`}
                style={{
                  width: diameter,
                  height: diameter,
                }}
              >
                <span className="activation-ring-label">LINE {lineNum}</span>
                <span className="activation-ring-stats">
                  {lineFilled}/{linePositions.length} • arrivals {arrivals}
                </span>
              </div>
            )
          })}
          ...
          {structure.lines.map((lineNum) =>
            structure.positions[lineNum].map((posNumber) => {
              const position =
                positions.find((item) => Number(item.number) === Number(posNumber)) || {
                  number: posNumber,
                  line: lineNum,
                }

              const angle = structure.customAngles?.[lineNum]?.[posNumber] ?? -90
              const coords = getPositionOnAngle(angle, ringRadiiPx[lineNum], centerX, centerY)
              const nodeType = getActivationOrbitNodeType(position, viewer)
              const amount = Number(position?.viewerReceiptBreakdown?.totalLiquid || 0)

              return (
                <button
                  key={`position-${posNumber}`}
                  type="button"
                  className={`activation-planet-node ${nodeType} ${orbitType.toLowerCase()}`}
                  style={{
                    left: coords.x,
                    top: coords.y,
                  }}
      ...
      <div className="activation-orbit-legend">
        <span><i className="legend-dot mine" /> You</span>
        <span><i className="legend-dot downline" /> Downline</span>
...
const LevelOrbitModal = ({
  ...
  return (
    <div className="activation-overlay activation-orbit-overlay" role="dialog" aria-modal="true">
      <div className="activation-modal activation-orbit-modal">
        <div className="activation-modal__top">
          <div>
            <h3 className="activation-modal__title">
              Level {level} {orbitType} Orbit
            </h3>
            <p className="activation-modal__text">
              Real-time structure for this exact activation level.
            </p>
          </div>
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close orbit preview"
          >
            <FaTimesCircle />
          </button>
        </div>
        ...
        {loading ? (
          <div className="activation-orbit-loading">
            <div className="spinner" />
            <p>Loading Level {level} orbit data...</p>
          </div>
        ) : snapshot ? (
          <ActivationOrbitGalaxy
            level={level}
            viewer={viewer}
            snapshot={snapshot}
          />
        ) : (
          <div className="activation-orbit-empty">
...
const ActivationCenterPage = () => {
  ...
  const { refCode } = useParams()  // ADDED

  const viewer = subjectAddress || account

  // ==================== REFERRAL SYSTEM WITH SECURE LOCK ====================
  const [myShortCode, setMyShortCode] = useState('')
  const [myReferralLink, setMyReferralLink] = useState('')
  const [activationVisitorId, setActivationVisitorId] = useState('')
  const [securedReferral, setSecuredReferral] = useState(null)
  const [referralLockLoading, setReferralLockLoading] = useState(false)
  const [referralLockError, setReferralLockError] = useState('')
  ...
  // ADDED: new referral states
  const [referralJourneyStatus, setReferralJourneyStatus] = useState('idle')
  const [referralRequiresChoice, setReferralRequiresChoice] = useState(false)
  const [previousBrowserReferrer, setPreviousBrowserReferrer] = useState('')
  ...
  const [selectedOrbitLevel, setSelectedOrbitLevel] = useState(null)
  ...
  useEffect(() => {
    setActivationVisitorId(getOrCreateActivationVisitorId())
  }, [])

  // REPLACED: wallet-change clearing effect
  useEffect(() => {
    /*
      Wallet switch rule:
      - do not silently attach old browser referral to a new wallet
      - do not erase URL/browser invitation visually while backend checks
      - final registration will still resolve through backend
    */
    setSecuredReferral(null)
    setRegistrationReferrer('')
    setResolvedReferrerShortCode('')
    setReferralLockError('')

    if (referrerInputDisplay || incomingReferrer) {
      const remembered = referrerInputDisplay || incomingReferrer
      setPreviousBrowserReferrer(remembered)
      setReferralRequiresChoice(Boolean(account))
      setReferralJourneyStatus(account ? 'wallet_changed_choice_required' : 'browser_remembered')
      setResolvedReferrerStatus(
        account
          ? `New wallet detected. Previous browser invitation ${remembered} is available, but it will only be used if you choose it.`
          : `Previous invitation ${remembered} is available for registration.`
      )
    } else {
      setReferralRequiresChoice(false)
      setReferralJourneyStatus(account ? 'wallet_checking' : 'idle')
      setResolvedReferrerStatus('')
    }
  }, [account])

  // ==================== REFERRAL STATE HELPERS ====================
  ...
  // ==================== REFERRAL STATE HELPERS ====================
  const isWalletReferralLocked =
    securedReferral?.locked &&
    securedReferral?.scope === 'wallet' &&
    !securedReferral?.pendingWalletBinding

  const isPendingVisitorReferral =
    securedReferral?.locked &&
    securedReferral?.scope === 'visitor' &&
    Boolean(securedReferral?.pendingWalletBinding)

  const currentReferralCode =
    securedReferral?.referrerCode ||
    referrerInputDisplay ||
    incomingReferrer ||
    ''

  const getReferralLockLabel = () => {
    if (isWalletReferralLocked) {
      return `This wallet is locked to ${securedReferral.referrerCode} for ${getReferralDaysLeftText(securedReferral.daysLeft)}.`
    }

    if (isPendingVisitorReferral) {
      return `You arrived through ${securedReferral.referrerCode}. This referrer is pending and will be locked only if this wallet registers.`
    }

        if (referralLockError) {
        return referralLockError
      }

      if (resolvedReferrerStatus) {
        return resolvedReferrerStatus
      }

      if (referrerInputDisplay) {
        return 'Referral input captured. It will be verified before registration.'
      }

      return 'No referrer added. You will be placed under the system ID automatically.'
  }
  
// REPLACED: applySecuredReferralToState
const applySecuredReferralToState = useCallback((referralData) => {
  if (!referralData?.locked) {
    setSecuredReferral(null)
    setRegistrationReferrer('')
    setResolvedReferrerShortCode('')

    if (referralData?.referralReservedForAnotherWallet) {
      setReferralJourneyStatus('wallet_changed_choice_required')
      setReferralRequiresChoice(true)
      setResolvedReferrerStatus(
        referralData.message ||
          'This browser invitation is already reserved for another wallet. Choose another referrer or continue with the system ID.'
      )
      return
    }

    if (referralData?.referralConsumed) {
      setReferralJourneyStatus('consumed')
      setReferralRequiresChoice(false)
      setIncomingReferrer('')
      setReferrerInputDisplay('')
      setPreviousBrowserReferrer('')
      setResolvedReferrerStatus(
        referralData.message ||
          'This browser invitation has already been used. You can enter another referrer or continue with the system ID.'
      )
      return
    }

    setReferralJourneyStatus('no_referrer')
    setReferralRequiresChoice(false)
    setResolvedReferrerStatus('')
    return
  }
  ...
  const isWalletLock =
    referralData.scope === 'wallet' &&
    !referralData.pendingWalletBinding

  const isPendingVisitorLock =
    referralData.scope === 'visitor' &&
    Boolean(referralData.pendingWalletBinding)

  setSecuredReferral(referralData)
  setIncomingReferrer(referralData.referrerCode || '')
  setReferrerInputDisplay(referralData.referrerCode || '')
  setResolvedReferrerShortCode(referralData.referrerCode || '')
  setPreviousBrowserReferrer(referralData.referrerCode || '')

  setRegistrationReferrer(isWalletLock ? referralData.referrerWallet || '' : '')

  if (isWalletLock) {
    setReferralJourneyStatus('wallet_locked')
    setReferralRequiresChoice(false)
    setResolvedReferrerStatus(
      `Wallet referrer locked: ${referralData.referrerCode}.`
    )
    return
  }

  if (isPendingVisitorLock) {
    setReferralJourneyStatus('ready_for_registration')
    setReferralRequiresChoice(false)
    setResolvedReferrerStatus(
      `Invitation ready: ${referralData.referrerCode}. It will be securely locked when this wallet registers.`
    )
    return
  }

  setReferralJourneyStatus(referralData.restored ? 'browser_remembered' : 'url_detected')
  setReferralRequiresChoice(false)
  setResolvedReferrerStatus(
    referralData.restored
      ? `Previous invitation restored: ${referralData.referrerCode}.`
      : `Invitation detected: ${referralData.referrerCode}.`
  )
}, [])

  // Fetch existing secured referral
  const fetchSecuredReferral = useCallback(async () => {
    if (!activationVisitorId && !account) return null

    const query = new URLSearchParams({
      visitorId: activationVisitorId || '',
      walletAddress: account || '',
    })

    const res = await fetch(`${API_BASE}/api/referral/lock?${query.toString()}`)
    const data = await res.json()

    if (!res.ok || data.success === false) {
      throw new Error(data.message || 'Failed to fetch secured referral.')
    }

    return data
  }, [activationVisitorId, account])

  // Secure a referral with the system
  const secureReferralWithSystem = useCallback(async (ref, source = 'referral_link') => {
    if (!ref) return null
    if (!activationVisitorId && !account) return null

    setReferralLockLoading(true)
    setReferralLockError('')

    try {
      const res = await fetch(`${API_BASE}/api/referral/lock`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          visitorId: activationVisitorId,
          walletAddress: account || '',
          ref: normalizeReferralInput(ref),  // CHANGED: added normalizeReferralInput
          source,
        }),
      })
      ...
      if (!res.ok || data.success === false) {
        throw new Error(data.message || 'Failed to secure referral.')
      }

      applySecuredReferralToState(data)
      return data
    } catch (err) {
      setReferralLockError(err.message || 'Referral security failed.')
      setResolvedReferrerStatus(err.message || 'Referral security failed.')
      return null
    } finally {
      setReferralLockLoading(false)
    }
  }, [activationVisitorId, account, applySecuredReferralToState])

  // REPLACED: fetchMyReferralCode
  const fetchMyReferralCode = useCallback(async () => {
    ...
    try {
      ...
      setMyReferralLink(data.fullLink || `https://finfreedomnetwork.io/ref/${data.shortCode}`)
      setReferredByCode(data.referredByCode || '')
  ...
  // Sync referral from URL with secure system
  useEffect(() => {
    if (!activationVisitorId && !account) return

    let cancelled = false

    const syncReferral = async () => {
      const urlRef = extractReferralFromUrl(location, refCode)  // CHANGED: added refCode

      setReferralLockLoading(true)
      setReferralLockError('')

      try {
        const existingSecureReferral = await fetchSecuredReferral()

        if (cancelled) return

        if (existingSecureReferral) {
  /*
    Important:
    Apply both locked and non-locked backend responses.

    This matters because backend may return:
    {
      locked: false,
      referralConsumed: true
    }

    In that case, the frontend must clear the input and must NOT recreate
    the old URL referral again.
  */
  applySecuredReferralToState(existingSecureReferral)

      if (existingSecureReferral.referralConsumed) {
        return
      }

      if (existingSecureReferral.locked) {
        if (
          urlRef &&
          existingSecureReferral.referrerCode &&
          urlRef.toLowerCase() !== String(existingSecureReferral.referrerCode).toLowerCase()
        ) {
          setResolvedReferrerStatus(
            existingSecureReferral.scope === 'wallet'
              ? `This wallet already has a secured referrer for ${getReferralDaysLeftText(existingSecureReferral.daysLeft)}. New referral links cannot override it.`
              : `This browser already has a pending referrer for ${getReferralDaysLeftText(existingSecureReferral.daysLeft)}. It will only become wallet-secured if this wallet registers.`
          )
        }
    ...
    if (urlRef) {
      await secureReferralWithSystem(urlRef, 'referral_link')
    }
      } catch (err) {
        if (!cancelled) {
          setReferralLockError(err.message || 'Could not check referral security.')
        }
      } finally {
        if (!cancelled) {
          setReferralLockLoading(false)
    ...
    activationVisitorId,
    account,
    refCode,  // ADDED to dependency array
    fetchSecuredReferral,
    applySecuredReferralToState,
    secureReferralWithSystem,
  ])
  ...
  const handleViewLevelOrbit = useCallback(async (level) => {
    if (!viewer) return

    setSelectedOrbitLevel(level)
    setSelectedOrbitSnapshot(null)
    setSelectedOrbitLoading(true)
    setIsLevelOrbitModalOpen(true)

    try {
      const snapshot = await fetchOrbitLevelSnapshotApi(viewer, level)
      setSelectedOrbitSnapshot(snapshot)
    } catch (err) {
      console.error(`Failed to load orbit for level ${level}:`, err)
      setTxStatus({
        loading: false,
        hash: null,
        error: `Could not load Level ${level} orbit data. Please try again.`,
      })
    } finally {
  ...
  }, [isEligibilityModalOpen, isNextActionModalOpen, isRegistrationModalOpen, showSecurityNotice, isLevelOrbitModalOpen])
  ...
  const handleRegisterFromModal = async () => {
    try {
      let finalSecureReferral = null

      /*
        First ask backend for the current truth.

        If this wallet already has a wallet lock, backend returns:
        scope: 'wallet'

        If only the browser has referral memory, backend returns:
        scope: 'visitor'
        pendingWalletBinding: true
      */
      const latestReferral = await fetchSecuredReferral()

      if (
        latestReferral?.locked &&
        latestReferral.scope === 'wallet' &&
        !latestReferral.pendingWalletBinding
      ) {
        finalSecureReferral = latestReferral
        applySecuredReferralToState(latestReferral)
      } else {
        /*
          If it is pending visitor referral, we now convert it into a wallet lock
          because the user is actually registering this wallet.
        */
        const refToLock =
          latestReferral?.locked && latestReferral?.referrerCode
            ? latestReferral.referrerCode
            : normalizeReferralInput(referrerInputDisplay || incomingReferrer) || 'FIN-FREEDOM'  // CHANGED

        finalSecureReferral = await secureReferralWithSystem(refToLock, 'registration')
      }

      if (!finalSecureReferral?.locked || !finalSecureReferral.referrerWallet) {
        throw new Error('Could not secure referral before registration.')
      }

      setRegistrationReferrer(finalSecureReferral.referrerWallet)

      await handleCombinedRegisterAndActivateLevelOne(finalSecureReferral.referrerWallet)
    } catch (err) {
      setTxStatus({
        loading: false,
        hash: null,
        error: err.message || 'Could not verify referral security before registration.',
      })
    }
  }
  ...
  // ADDED: user choice handlers before ReferralCard
  const handleUsePreviousBrowserReferrer = async () => {
    const refToUse = normalizeReferralInput(previousBrowserReferrer || referrerInputDisplay || incomingReferrer)

    if (!refToUse) {
      setReferralRequiresChoice(false)
      setReferralJourneyStatus('no_referrer')
      setResolvedReferrerStatus('No previous invitation found. System ID will be used.')
      return
    }

    setReferrerInputDisplay(refToUse)
    setIncomingReferrer(refToUse)
    setReferralRequiresChoice(false)
    setReferralJourneyStatus('ready_for_registration')

    await secureReferralWithSystem(refToUse, 'manual_input')
  }

  const handleUseSystemReferrer = async () => {
    setSecuredReferral(null)
    setIncomingReferrer('')
    setReferrerInputDisplay('')
    setRegistrationReferrer('')
    setResolvedReferrerShortCode('')
    setReferralRequiresChoice(false)
    setReferralJourneyStatus('system_selected')
    setResolvedReferrerStatus('System ID will be used for registration.')
  }

  const handleEnterAnotherReferrer = () => {
    setSecuredReferral(null)
    setRegistrationReferrer('')
    setResolvedReferrerShortCode('')
    setReferralRequiresChoice(false)
    setReferralJourneyStatus('manual_entry')
    setResolvedReferrerStatus('Enter a referral ID, referral link, or wallet address below.')
  }

  // REPLACED: ReferralCard component
  const ReferralCard = () => {
    if (!isRegistered) {
      return (
    ...
    return (
      <section className="activation-referral-card">
        ...
        {myShortCode ? (
          <>
            <p className="activation-referral-card__text">
              Share your clean route. New users land directly in the Activation Center with your referral secured.
            </p>
            ...
            <div className="activation-referral-card__link">
              <code>{myReferralLink || `https://finfreedomnetwork.io/ref/${myShortCode}`}</code>
            </div>
          ...
          <p className="activation-referral-card__text">
            Your registration is confirmed. Referral access is being prepared by the indexer.
          </p>
        )}
      </section>
    )
  ...
  // ADDED: ReferralInvitationCard component
  const ReferralInvitationCard = () => {
    if (isRegistered) return null

    const hasReferrer = Boolean(currentReferralCode || previousBrowserReferrer)
    ...
    return (
      <section className={`activation-referral-card activation-referral-card--invitation ${hasReferrer ? 'has-referrer' : ''}`}>
        ...
        <p className="activation-referral-card__text">
          {getReferralLockLabel()}
        </p>

        {hasReferrer && (
          <div className="activation-referral-card__code">
            <span>{currentReferralCode || previousBrowserReferrer}</span>
          </div>
        )}

        {referralRequiresChoice && (
          <div className="activation-referral-card__actions">
            <button
              type="button"
              className="activation-referral-card__button"
              onClick={handleUsePreviousBrowserReferrer}
            >
              Use {previousBrowserReferrer || currentReferralCode}
            </button>

            <button
              type="button"
              className="activation-referral-card__button activation-referral-card__button--ghost"
              onClick={handleEnterAnotherReferrer}
            >
              Enter Another
            </button>
            ...
            <button
              type="button"
              className="activation-referral-card__button activation-referral-card__button--ghost"
              onClick={handleUseSystemReferrer}
            >
              Use System ID
            </button>
          </div>
    ...
    <section className="activation-page">
      ...
      <div className="activation-hero">
        ...
        <div className="activation-hero__visual glass-panel">
          ...
          {/* CHANGED: Referral card / invitation card */}
      ...
      <section className="activation-main-grid">
        ...
        <div className="activation-main-grid__right">
          ...
          {/* Registration Modal with secured referral */}
          {isRegistrationModalOpen && (
            <div className="activation-overlay" role="dialog" aria-modal="true">
              <div className="activation-modal">
                <div className="activation-modal__top">
                ...
                <div className="referrer-input-group">
                  <label className="referrer-label">
                    {isWalletReferralLocked
                      ? `Wallet referrer locked: ${referrerInputDisplay}`
                      : isPendingVisitorReferral
                        ? `Suggested referrer: ${referrerInputDisplay}`
                  ...
                  <input
                    ...
                    className={`referrer-input ${
                      isWalletReferralLocked ? 'is-locked' : isPendingVisitorReferral ? 'is-pending' : ''
                    }`}
                    placeholder="Paste referral ID, referral link, or wallet address"
                    value={referrerInputDisplay}
                    readOnly={isWalletReferralLocked}
                    onChange={(e) => {
                      if (isWalletReferralLocked) return

                      const value = normalizeReferralInput(e.target.value)  // CHANGED
                      setReferrerInputDisplay(value)
                      setIncomingReferrer(value)
                      setReferralRequiresChoice(false)  // ADDED
                      setReferralJourneyStatus(value ? 'manual_entry' : 'no_referrer')  // ADDED
                      ...
                      if (ethers.isAddress(value)) {
                        ...
                        setResolvedReferrerStatus('Wallet address detected. It will be secured when this wallet registers.')
                      } else {
                        setRegistrationReferrer('')
                        setResolvedReferrerStatus('Referral ID will be resolved and secured when this wallet registers.')
                      }
                    }}
                  />
                  ...
                  <p
                    className={`referrer-hint referrer-hint--important ${
                      isWalletReferralLocked
                        ? 'is-wallet-locked'
                        : isPendingVisitorReferral
                          ? 'is-pending'
                  ...
                  >
                    {isWalletReferralLocked
                      ? 'This wallet already has a secured referrer. It cannot be replaced during the lock period.'
                      : isPendingVisitorReferral
                        ? 'This referrer becomes validated only when this wallet registers.'
                        : 'Your referrer will be verified and secured by the system before registration.'}
                  </p>
                </div>
          ...
          {isLevelOrbitModalOpen && (
            <LevelOrbitModal
              level={selectedOrbitLevel}
              viewer={viewer}
              snapshot={selectedOrbitSnapshot}
              loading={selectedOrbitLoading}
              onClose={() => {
...
// const extractReferralFromUrl = (location) => {
//   const params = new URLSearchParams(location.search)
...
//   useEffect(() => {
//   /*
//     When wallet changes, clear old referral display immediately.
//     The backend fetch will then restore only the correct state for the new wallet.

//     This prevents the previous wallet/browser referral from remaining visually
//     inside the input while the new wallet check is still loading.
//   */
//   setSecuredReferral(null)
//   setIncomingReferrer('')
//   setReferrerInputDisplay('')
...
// const applySecuredReferralToState = useCallback((referralData) => {
//   // Apply secured referral to state
//   if (!referralData?.locked) {
//     setSecuredReferral(null)
//     setIncomingReferrer('')
//     setReferrerInputDisplay('')
...
//     setResolvedReferrerStatus(
//       referralData?.referralConsumed || referralData?.referralReservedForAnotherWallet
//         ? referralData.message || 'This referral is not available for this wallet.'
//         : ''
//     )
...
//     /*
//       Important:
//       We can show the referrer wallet for display/preview,
//       but the final registration transaction must still call backend with source='registration'
//       before using it.
//     */
//     setRegistrationReferrer(referralData.referrerWallet || '')

//     if (isWalletLock) {
//       setResolvedReferrerStatus(
//         `This wallet is locked to ${referralData.referrerCode} for ${getReferralDaysLeftText(referralData.daysLeft)}.`
//       )
//       return
//     }
...
//     if (isPendingVisitorLock) {
//       setResolvedReferrerStatus(
//         `You arrived through ${referralData.referrerCode}. This referrer is pending and will be locked only if this wallet registers.`
//       )
//       return
//     }
...
//     setResolvedReferrerStatus(
//       referralData.restored
//         ? `Existing browser referrer restored for ${getReferralDaysLeftText(referralData.daysLeft)}.`
//         : `Referral saved for this browser for ${getReferralDaysLeftText(referralData.daysLeft)}.`
//     )
//   }, [])
...
//     const syncReferral = async () => {
//       const urlRef = extractReferralFromUrl(location)
...
//           latestReferral?.locked && latestReferral?.referrerCode
//             ? latestReferral.referrerCode
//             : referrerInputDisplay?.trim() || 'FIN-FREEDOM'
...
//       ) : (
//         <p className="activation-referral-card__text">
//           Complete registration to generate your personal referral link.
//         </p>
//       )}
//     </section>
//   )
...
//           {/* Referral Card moved to hero panel */}
//           {isRegistered && <ReferralCard />}
//         </div>
//       </div>
