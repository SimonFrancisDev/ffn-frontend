...
const ActivityPage = () => {
      ...
      for (let level = 1; level <= 10; level += 1) {
        ...
        if (isActive) {
          ...
          activationActivities.push({
            id: `activation-${level}`,
            type: 'activation',
            title: `Level ${level} Activated`,
            description: `Successfully activated Level ${level}`,
            amount: null,
            level: level,
            timestamp: realTimestamp || Math.floor(Date.now() / 1000) - (level * 86400 * 7),
            status: 'completed',
          });
        }
      }
  ...
  }, [contracts, account, normalizeTimestamp]);
      ...
      <div className="activity-main-grid">
        ...
        <div className="activity-main-grid__right">
          ...
          <section className="activity-sidebar-bottom glass-panel">
            <div className="activity-section-heading">
              <span className="activity-section-heading__eyebrow muted-text">Progress & Data</span>
              <h2 className="activity-section-heading__title">Level Status & Export</h2>
            </div>

            <div className="merged-bottom-grid">
              {/* Level Status */}
              <div className="merged-levels">
                <div className="merged-subheading">Your Activation Progress</div>
              ...
              <div className="merged-export">
                <div className="merged-subheading">Export Your History</div>
...
//   // ==================== FIXED TIMESTAMP HANDLER ====================
//   const normalizeTimestamp = useCallback((value) => {
//     if (!value) return null

//     // Handle ISO string (what your API returns)
//     if (typeof value === 'string') {
//       const date = new Date(value)
//       if (!Number.isNaN(date.getTime())) {
//         return Math.floor(date.getTime() / 1000)
...
//     const numeric = Number(value)
//     if (Number.isFinite(numeric) && numeric > 0) {
//       if (numeric > 1e12) return Math.floor(numeric / 1000)      // milliseconds
//       if (numeric > 1e15) return Math.floor(numeric / 1000000)   // microseconds
//       return Math.floor(numeric)
//     }
...
//   // ==================== REAL LEVEL ACTIVATION DATES ====================
//   const fetchLevelActivations = useCallback(async () => {
//     if (!contracts || !account) return;

//     try {
//       // 1. Get real level data from backend (may contain activation time)
//       let orbitLevelsData = [];
//       try {
//         const levelsResponse = await fetchOrbitLevelsApi(account);
//         orbitLevelsData = levelsResponse?.levels || levelsResponse || [];
...
//         if (isActive) {
//           // Try to find real activation timestamp from orbit levels data
//           const levelData = orbitLevelsData.find(l => l.level === level);
...
//             amount: null,
//             level: level,
//             timestamp: realTimestamp || Math.floor(Date.now() / 1000) - (level * 86400 * 7), // fallback: older dates
//             status: 'completed',
//           });
//         }
//       }
