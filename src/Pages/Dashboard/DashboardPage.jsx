...
//       const balanceEntries = await Promise.all(
//         targets.map(async ([key, address]) => {
//           const raw = await safeReadContract(
...
//       const participants = await safeReadContract(
...
//       if (contracts?.levelManager) {
//         const nftPool = await safeReadContract(
//           () => contracts.levelManager.nftPool(),
//           null
//         )
//         const opsWallet = await safeReadContract(
//           () => contracts.levelManager.operationsWallet(),
//           null
//         )
...
//         icon: PiggyBank,
//         iconColor: '#8b5cf6',
//         title: 'Escrow balance visible',
//         description: `$${formatNumber(contractBalances.ESCROW)} currently visible in escrow`,
//         time: formatRelativeTime(lastUpdated),
//         amount: Number(contractBalances.ESCROW || 0),
//       })
//     }
...
//     if (!growthData.series.length) return 1
//     return Math.max(...growthData.series.map((d) => Number(d.registrations || 0)), 1)
//   }, [growthData.series])
...
//             <div className="dashboard-hero__mini-card glass-panel">
//               <span className="dashboard-hero__mini-label soft-text">
//                 <PiggyBank size={12} /> Visible Core
//               </span>
//               <strong className="dashboard-hero__mini-value">
//                 ${formatUsdtString(publicSummary.visibleCoreBalanceUsdt)}
//               </strong>
//             </div>
//           </div>
...
//                 {growthData.series.slice(-7).map((item, i) => {
//                   const registrations = Number(item.registrations || 0)
//                   const height = Math.max((registrations / growthMax) * 100, 8)
...
//               {growthData.rangeDays}-day registration trend
//             </span>
//           </div>
//         </div>
//       </div>
...
//             <div className="growth-chart-full">
//               {growthData.series.length > 0 ? (
//                 <div className="growth-chart-bars">
//                   {growthData.series.slice(-7).map((item, i) => {
//                     const registrations = Number(item.registrations || 0)
//                     const earnings = Number(item.earningsLiquid || 0)
//                     const height = Math.max((registrations / growthMax) * 100, 10)

//                     return (
//                       <div key={`${item.date || i}-${i}`} className="growth-chart-bar-wrapper">
//                         <div
//                           className="growth-chart-bar"
//                           style={{ height: `${height}%` }}
//                           title={`${item.date}: ${registrations} registrations • $${formatNumber(
//                             earnings
//                           )}`}
//                         />
//                         <span className="growth-chart-label">{item.date?.slice(5)}</span>
//                       </div>
//                     )
//                   })}
//                 </div>
...
//           <section className="dashboard-notices glass-panel">
//             <div className="dashboard-section-heading">
//               <span className="dashboard-section-heading__eyebrow soft-text">Updates</span>
//               <h2 className="dashboard-section-heading__title">Announcements</h2>
//             </div>
...
//               <h2 className="dashboard-section-heading__title">System Health</h2>
//             </div>
...
//         .dashboard-hero__growth-sparkline {
//           margin-top: 16px;
//           padding-top: 16px;
